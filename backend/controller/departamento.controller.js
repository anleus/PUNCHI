...
const Departamento = require('../models/Jornada');
...
departamentFunctions.getDepartamentoById = async (req, res, next) => {
    const deparamentos = await Departamento.findById(req.params.id).catch(err => console.log(err));
    if(deparamentos)
        res.status(200).json(deparamentos);
    else
        res.status(404).json('Not Found');
}

departamentFunctions.addDeparamento = async (req, res, next) => {
    ...
    deparamento.save()
        .then(() => res.json({status: 'Departamento saved'}))
}
...
departamentFunctions.updateDepartamento = async (req, res, next) => {
    ...
    Departamento.findByIdAndUpdate(req.body.id, {$set: deparamento}, {new: true})
        .then(() => {
            res.status(200); 
